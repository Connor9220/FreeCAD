...
class ToolBitBrowserWidget(QtGui.QWidget):
    ...
    def _on_delete_requested(self):
        ...
        reply = QMessageBox.question(
            ...
            FreeCAD.Qt.translate("CAM", "Are you sure you want to delete the selected toolbit(s)? This is not reversible. The toolbits will be removed from disk and from all libraries that contain them."),
        ...
        for uri_string in uris:
            try:
                ...
                for library in libraries_to_update:
                    ...
                    Path.Log.info(f"Removed toolbit {toolbit_uri.asset_id} from library {library.label}")
        ...
        if deleted_count > 0:
            Path.Log.info(f"Deleted {deleted_count} toolbit(s) and updated {len(libraries_modified)} libraries.")
            self.refresh()
