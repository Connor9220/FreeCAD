...
class LibraryBrowserWidget(ToolBitBrowserWidget):
    ...
    def _set_state(self, selection_data):
        ...
        if library_uri_str:
            try:
                ...
                Path.Log.info(f"BROWSER: Attempting to load library '{library_uri.asset_id}' from store '{self._store_name}' with depth=1")
                library = self._asset_manager.get(library_uri, store=self._store_name, depth=1)
                self.set_current_library(library)
            except FileNotFoundError:
                Path.Log.error(f"Library {library_uri_str} not found in store '{self._store_name}'.")
    ...
    def refresh(self):
        ...
        if self.current_library:
            ...
            try:
                Path.Log.info(f"BROWSER: Loading library '{library_uri.asset_id}' from store '{self._store_name}' with depth=1")
                library = cast(
                    Library, self._asset_manager.get(library_uri, store=self._store_name, depth=1)
                )
            except FileNotFoundError:
