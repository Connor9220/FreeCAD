...
class AssetManager:
    ...
    async def _fetch_asset_construction_data_recursive_async(
        ...
        if uri.asset_type == 'library':
            logger.info(f"LIBRARY FETCH: Loading library '{uri.asset_id}' with depth={depth} from stores {store_names}")
        ...
        if uri.asset_type == 'toolbit':
            logger.info(f"TOOLBIT SEARCH: Looking for toolbit '{uri.asset_id}' in stores: {store_names}")
        ...
        for current_store_name in store_names:
            ...
            if uri.asset_type == 'toolbit':
                ...
                logger.info(f"TOOLBIT SEARCH: Checking store '{current_store_name}' at path: {store_path}")

            try:
                raw_data = await store.get(uri)
                found_store_name = current_store_name
                if uri.asset_type == 'toolbit':
                    logger.info(f"TOOLBIT FOUND: '{uri.asset_id}' found in store '{found_store_name}'")
                logger.debug(
                ...
                if uri.asset_type == 'toolbit':
                    logger.info(f"TOOLBIT SEARCH: '{uri.asset_id}' NOT found in store '{current_store_name}'")
                logger.debug(
        ...
        if raw_data is None or not found_store_name:
            if uri.asset_type == 'toolbit':
                logger.warning(f"TOOLBIT NOT FOUND: '{uri.asset_id}' not found in any of the stores: {store_names}")
            return None  # Asset not found in any store

        if depth == 0:
            if uri.asset_type == 'library':
                logger.warning(f"LIBRARY SHALLOW: Library '{uri.asset_id}' loaded with depth=0 - no dependencies will be resolved")
    ...
    def _build_asset_tree_from_data_sync(
        ...
        if construction_data.dependencies_data is not None:
            ...
            if construction_data.uri.asset_type == 'library':
                logger.info(f"LIBRARY DEPS: Resolving {len(construction_data.dependencies_data)} dependencies for library '{construction_data.uri.asset_id}'")
            ...
            for (
                ...
                if dep_uri.asset_type == 'toolbit':
                    logger.info(f"TOOLBIT DEP: Resolving dependency '{dep_uri.asset_id}' for library '{construction_data.uri.asset_id}'")
                ...
                try:
                    ...
                    if dep_uri.asset_type == 'toolbit':
                        if dep:
                            logger.info(f"TOOLBIT DEP: Successfully resolved '{dep_uri.asset_id}' -> {type(dep).__name__}")
                        else:
                            logger.warning(f"TOOLBIT DEP: Dependency '{dep_uri.asset_id}' resolved to None")
            ...
            if construction_data.uri.asset_type == 'library':
                toolbit_deps = [uri for uri in resolved_dependencies.keys() if uri.asset_type == 'toolbit']
                logger.info(f"LIBRARY DEPS: Resolved {len(resolved_dependencies)} total dependencies ({len(toolbit_deps)} toolbits) for library '{construction_data.uri.asset_id}'")
        else:
            # Log when dependencies_data is None
            if construction_data.uri.asset_type == 'library':
                logger.warning(f"LIBRARY NO DEPS: Library '{construction_data.uri.asset_id}' has dependencies_data=None - was loaded with depth=0")
        ...
        try:
            ...
            if construction_data.uri.asset_type == 'library':
                ...
                logger.info(f"LIBRARY INSTANTIATE: Creating library '{construction_data.uri.asset_id}' with {dep_count} dependencies")
    ...
    def get(
        ...
        if asset_uri_obj.asset_type == 'library':
            logger.info(f"LIBRARY GET: Request for library '{asset_uri_obj.asset_id}' with depth={depth}")
        elif asset_uri_obj.asset_type == 'toolbit':
            logger.info(f"TOOLBIT GET: Direct request for toolbit '{asset_uri_obj.asset_id}' with depth={depth} from stores {stores_list}")
