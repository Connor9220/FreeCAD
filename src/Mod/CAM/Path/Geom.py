...
def filterArcs(cmds, deflection=None):
    ...
    This is a convenience wrapper around the C++ Path.filterArcs() implementation.
    Accepts either a Path object or a list of Path.Command objects.

    Note: Most code doesn't need to call this explicitly - paths generated by
    Path.fromShapes() automatically have arc filtering enabled.
    """
    if not deflection:
    ...
    # Handle Path objects
    if isinstance(cmds, Path.Path):
        cmds.filterArcs(deflection)
        return cmds

    # Handle command lists - convert to Path, filter, return commands
    if isinstance(cmds, list):
        path = Path.Path(cmds)
        path.filterArcs(deflection)
        return path.Commands

    raise TypeError("filterArcs expects a Path object or list of Path.Command objects")
