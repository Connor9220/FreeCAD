...
class LinearAxis:
    ...
    def __post_init__(self):
        ...
        if self.min_limit >= self.max_limit:
            Path.Log.warning(f"LinearAxis {self.name}: min_limit ({self.min_limit}) >= max_limit ({self.max_limit})")
        
        # Validate velocity
        if self.max_velocity <= 0:
            Path.Log.warning(f"LinearAxis {self.name}: max_velocity must be positive, got {self.max_velocity}")
...
class RotaryAxis:
    ...
    def __post_init__(self):
        ...
        # Validate limits
        if self.min_limit >= self.max_limit:
            Path.Log.warning(f"RotaryAxis {self.name}: min_limit ({self.min_limit}) >= max_limit ({self.max_limit})")
        
        # Validate velocity
        if self.max_velocity <= 0:
            Path.Log.warning(f"RotaryAxis {self.name}: max_velocity must be positive, got {self.max_velocity}")
...
class Machine:
    ...
    def __post_init__(self):
        ...
        if self.configuration_units not in ["metric", "imperial"]:
            raise ValueError(f"configuration_units must be 'metric' or 'imperial', got '{self.configuration_units}'")
